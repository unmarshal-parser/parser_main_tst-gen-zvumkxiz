...
func (a PairCreatedEventTracker) CreatePairCreated(txLog blockchain.ReceiptLog, ethClient *EthBlockChainRPCWithRetry, u *UnmarshalSDKWrapper) (PairCreatedEvent, error) {
		...
		errs.Go(func() error {
			return func(i int) error {
				...
				case GetPairCreatedEventHash():
					event, err := pairCreatedEventTracker.CreatePairCreated(txLog.(blockchain.ReceiptLog), ethClient, u)
					if err != nil {
...
func syncMethods(orm *gorm.DB, syncDB SyncDB, rpcClient *EthBlockChainRPCWithRetry, config IndexerConfig, ethClient *ethclient.Client, chainID string, unmarshalSDKWrapper *UnmarshalSDKWrapper) {
	...
	go func() {
		...
		for true {
			...
			if err != nil {
				log.WithField("Location", "Sync Methods").Error("Failed to get Top Block")
				panic(err)
			}
			highestBlockNumber -= config.LagToHighestBlock
